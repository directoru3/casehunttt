...
export default function CaseCard({ caseData, onClick }: CaseCardProps) {
  return (
    <div
      onClick={onClick}
      className="group relative bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl overflow-hidden cursor-pointer transition-all duration-300 hover:shadow-2xl hover:shadow-blue-500/20 active:scale-95 touch-manipulation"
    >
      <div className="aspect-[3/4] relative overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-3 md:p-4 z-20">
          <h3 className="text-white font-bold text-sm md:text-lg mb-1 md:mb-2 line-clamp-1">{caseData.name}</h3>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1 md:gap-2 bg-blue-500/20 backdrop-blur-sm px-2 md:px-3 py-1 md:py-1.5 rounded-lg border border-blue-500/30">
              ...
              <span className="text-blue-300 font-bold text-sm md:text-base">{caseData.price}</span>
            </div>
          </div>
        </div>
